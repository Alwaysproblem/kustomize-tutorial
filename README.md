...
some basic example and tutorial for this tool of Customization of kubernetes YAML configurations

## installation

- [kustomize](https://github.com/kubernetes-sigs/kustomize/blob/master/docs/INSTALL.md) (need version >= 2.1, tested with v3.6.1)
...
- startup
  ...
  ```bash
  $ git clone https://github.com/Alwaysproblem/kustomize-tutorial
  ...
  - add PATH `export PATH=$PATH:$(pwd)` so that we can use kustomize directly

- directory structure
  - traditinal (version <= 2.0.4)
    - example from kustomize github
  ...
  - kustomization.yaml
    - [Fields](https://github.com/kubernetes-sigs/kustomize/blob/master/docs/fields.md)
    - **Note**: base directory is a relative one for the path that you put into the `kustomize build <path>`
  ...
  - yaml file for patchesStrategicMerge (not recommended)
    - the `apiVersion` `kind` `metadata.name` should be the same as the base yaml especially metadata.name

    ```yaml
    apiVersion: apps/v1
    kind: Deployment
    metadata:
      name: nginx # here it should be the same as the base.
    spec:
      template:
        spec:
          containers:
  ...
  - [example](tutorialDoc/patchJson6902.yaml) for [Json6902](https://tools.ietf.org/html/rfc6902)
    ...
    - example
      ...
      ```bash
      # assume that you are under kustomize-tutorial directory
      $ kustomize build overlays/patchJson6902/ > out/output.yaml
      ```

## Reference

- [mpi operator for kubeflow](https://www.kubeflow.org/docs/components/training/mpi/)
